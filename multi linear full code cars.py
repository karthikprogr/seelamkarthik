...
# Calculate Cooks distance for the new model
(c_V, _) = model_influence_new.cooks_distance

# Now you can find the index and value of the influencer
...
#discard the data points which are influencers and reasign the row number (rest_index())
car1=df1.drop(df1.index[[70,76]],axis=0).reset_index()
...
# Assign the model to final_ml_v
final_ml_v = model_new

# Now you can access rsquared and aic
(final_ml_v.rsquared,final_ml_v.aic)

cars_new=df1.drop(df1.index[[76,77]],axis=0).reset_index() # Change 'rest_index' to 'reset_index'
cars_new

#exclude variable "VOL" and generate r-squared and AIC values
final_ml_v=smf.ols('MPG~VOL+SP+HP',data=df1).fit()
(final_ml_v.rsquared,final_ml_v.aic)

final_ml_v=smf.ols('MPG~WT+SP+HP',data=df1).fit()
(final_ml_v.rsquared,final_ml_v.aic) #overall features contributing
...
(np.argmax(c_V),np.max(c_V))
car2=df.drop(df.index[[76,77]],axis=0)
car2

#
car3=car2.rest_index()